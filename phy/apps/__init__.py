...
def phycli(ctx):
    ...
    add_default_handler(
        level="DEBUG" if DEBUG else "INFO", logger=logging.getLogger("phy")
    )
    add_default_handler(
        level="DEBUG" if DEBUG else "INFO", logger=logging.getLogger("phylib")
    )
    add_default_handler(
        level="DEBUG" if DEBUG else "INFO", logger=logging.getLogger("mtscomp")
    )
...
@click.argument(
    "subdirs", nargs=-1, type=click.Path(exists=True, file_okay=False, dir_okay=True)
)
