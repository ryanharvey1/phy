...
def test_scatter_markers(qtbot, canvas_pz):
    ...
    _test_visual(
        qtbot, canvas_pz, ScatterVisual(marker="vbar"), x=x, y=y, data_bounds="auto"
    )
...
def test_uniform_plot_2(qtbot, canvas_pz):
    y = 0.2 * np.random.randn(10)
    _test_visual(
        qtbot, canvas_pz, UniformPlotVisual(), y=y, masks=0.5, data_bounds="auto"
    )
...
def test_uniform_plot_list(qtbot, canvas_pz):
    ...
    _test_visual(
        qtbot,
        canvas_pz,
        UniformPlotVisual(color=(1.0, 0.0, 0.0, 1.0)),
        y=y,
        masks=[0.1, 0.9],
    )
...
def test_image_2(qtbot, canvas):
    ...
    _test_visual(
        ...
        canvas,
        ImageVisual(),
        image=np.random.uniform(low=0.5, high=0.9, size=(n, n, 4)),
    )
...
def test_line_0(qtbot, canvas_pz):
    ...
    _test_visual(
        qtbot, canvas_pz, LineVisual(), pos=pos, color=color, data_bounds=[-1, -1, 1, 1]
    )
...
def test_text_3(qtbot, canvas_pz):
    ...
    _test_visual(
        ...
        pos=[(0, 0)] * 10,
        text=text,
        anchor=[(1, -1 - 2 * i) for i in range(5)]
        + [(-1 - 2 * i, 1) for i in range(5)],
    )
