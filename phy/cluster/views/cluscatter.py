...
class ClusterScatterView(
    MarkerSizeMixin, BaseColorView, BaseGlobalView, ManualClusteringView
):
    ...
    def __init__(self, cluster_ids=None, cluster_info=None, bindings=None, **kwargs):
        ...
        self.canvas.add_visual(
            self.label_visual, exclude_origins=(self.canvas.panzoom,)
        )
    ...
    def get_clusters_data(self, cluster_ids):
        ...
        return {
            cluster_id: self.get_cluster_data(cluster_id) for cluster_id in cluster_ids
        }
    ...
    def prepare_position(self):
        ...
        x = np.array(
            [
                self.cluster_data[cluster_id]["x_axis"] or 0.0
        ...
        y = np.array(
            [
                self.cluster_data[cluster_id]["y_axis"] or 0.0
                for cluster_id in self.all_cluster_ids
            ]
        )
    ...
    def prepare_size(self):
        ...
        size = np.array(
            [
                self.cluster_data[cluster_id]["size"] or 1.0
                for cluster_id in self.all_cluster_ids
            ]
        )
    ...
    def attach(self, gui):
        ...
        self.actions.add(
            self.set_x_axis, prompt=True, prompt_default=lambda: self.x_axis
        )
        self.actions.add(
            self.set_y_axis, prompt=True, prompt_default=lambda: self.y_axis
        )
