...
def test_cluster_scatter_view_1(qtbot, tempdir, gui):
    ...
    v = ClusterScatterView(
        cluster_info=cluster_info, cluster_ids=cluster_ids, bindings=bindings
    )
    v.add_color_scheme(
    ...
    mouse_click(
        qtbot, v.canvas, pos=(w * 0.3, h * 0.3), button="Left", modifiers=("Control",)
    )
    mouse_click(
        qtbot, v.canvas, pos=(w * 0.7, h * 0.3), button="Left", modifiers=("Control",)
    )
    mouse_click(
        qtbot, v.canvas, pos=(w * 0.7, h * 0.7), button="Left", modifiers=("Control",)
    )
    mouse_click(
        qtbot, v.canvas, pos=(w * 0.3, h * 0.7), button="Left", modifiers=("Control",)
    )
