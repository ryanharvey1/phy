...
def _cross_correlogram(
    ...
    """
    Compute discrete cross-correlogram of two time series.

    Parameters
    ----------
    t1 : array
        First time series (reference events).
    t2 : array
        Second time series (spike times).
    binsize : float
    ...
    -------
    C : array
        Cross-correlogram values (firing rate).
    """
    ...
    # Normalize by the total observation time and bin size
    C = C / (nt1 * binsize)
    return C
...
def _compute_psth(
    ...
    for spike_times in spike_trains:
        if len(spike_times) > 0 and len(events) > 0:
            psth = _cross_correlogram(events, spike_times, bin_width, n_bins)
        else:
...
class EventPsthView(ScalingMixin, ManualClusteringView):
    ...
    around specific events loaded from a .mat file in the current directory.

    Constructor
    -----------

    get_spike_times : function
        Function that maps cluster_id to spike times array.
    """
    ...
    def _load_events(self):
        ...
        try:
            ...
            # Use the last event key (following original pattern)
    ...
    def get_clusters_data(self, load_all=None):
        ...
        psth_data, time_bins = _compute_psth(
