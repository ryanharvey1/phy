...
class GUIState(Bunch):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._local_keys = (
            local_keys or []
        )  # A list of strings with full paths key1.key2
        if self._local_path:
    ...
    def _copy_default_state(self):
        ...
        if self._default_state_path and self._default_state_path.exists():
            logger.debug(
                "The GUI state file `%s` doesn't exist, creating a default one...",
                self._path,
            )
            ...
            logger.warning(
                "Could not copy non-existing default state file %s.",
                self._default_state_path,
            )
