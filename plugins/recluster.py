        ...
        def on_gui_ready(sender, gui):
            ...
            def Recluster_Local_PCAs():
                ...
                cmd += [
                    ...
                    "-MinClusters",
                    "20",
                ]  # ,"-MinClusters",'2',"-MaxClusters",'12'   ,"-MaxClusters",'12',"-MaxClusters",'12'
            ...
            def Recluster_Local_PCAs_all():
                ...
                """Relaunch KlustaKwik on selected clusters."""
                # Selected clusters.
                cluster_ids_iter = sorted(
                    controller.supervisor.shown_cluster_ids.copy()
                )
                for cluster_ids in cluster_ids_iter:
                    # skip if already noise or mua
                    if (
                        controller.supervisor.get_cluster_info(cluster_ids)["group"]
                        == "noise"
                    ) | (
                        controller.supervisor.get_cluster_info(cluster_ids)["group"]
                        == "mua"
            ...
            def K_means_clustering(kmeanclusters):
                ...
                print("testing...")
                data2 = data2[:, np.var(data2, axis=0) > 1e-6]
                whitened = whiten(data2)
                clusters_out, label = kmeans2(whitened, kmeanclusters, minit="random")
